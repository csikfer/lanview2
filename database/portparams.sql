...
CREATE OR REPLACE FUNCTION set_str_port_param(pid bigint, txtval text, tname text DEFAULT 'text') RETURNS reasons AS $$
DECLARE
    type_id bigint;
BEGIN
...
CREATE OR REPLACE FUNCTION set_bool_port_param(pid bigint, boolval boolean, tname text DEFAULT 'boolean' ) RETURNS reasons AS $$
BEGIN
    RETURN set_str_port_param(pid, boolval::text, tname);
END
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION set_int_port_param(pid bigint, intval bigint, tname text DEFAULT 'bigint') RETURNS reasons AS $$
BEGIN
    RETURN set_str_port_param(pname, intval::text, tname);
END
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION set_interval_port_param(pid bigint, ival interval, tname text DEFAULT 'interval') RETURNS reasons AS $$
BEGIN
    RETURN set_str_port_param(pid, ival::text, tname);
END
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION get_str_port_param(pid bigint, tname text DEFAULT 'text') RETURNS text AS $$
DECLARE
    res text;
    type_id bigint;
BEGIN
...
CREATE OR REPLACE FUNCTION get_bool_port_param(pid bigint, tname text DEFAULT 'boolean') RETURNS boolean AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION get_int_port_param(pid bigint, tname text DEFAULT 'bigint') RETURNS bigint AS $$
BEGIN
    RETURN get_str_port_param(pid,tname)::bigint;
END
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION get_interval_port_param(pid bigint, tname text  DEFAULT 'interval') RETURNS interval AS $$
BEGIN
